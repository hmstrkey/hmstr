...
function createBlock(block) {
    ...
    element.style.width = `${block.width * blockSize}px`;
    element.style.height = `${block.height * blockSize}px`;
    element.style.top = `${block.y * blockSize}px`;
    element.style.left = `${block.x * blockSize}px`;
    return element;
}
...
function animateBlocks(blocks) {
    ...
    blocks.forEach(block => {
        ...
        // Adding animation
        setTimeout(() => {
            blockElement.style.transform = `translate(${block.x * blockSize}px, ${block.y * blockSize}px)`;
        }, 100); // Delay for animation effect
    });
}

// Example usage with a specific date
function displayMap(date) {
    const map = maps[date];
    if (map) {
        animateBlocks(map.blocks);
        // Optionally display key and exitPosition here
    }
}

// Display a specific map
displayMap('2024-07-21');
